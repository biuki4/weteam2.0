		...
		<view class="game-container">
			<view style="background-color: #fff;">
				<view class="game-title padding-LR20 fontWeight550">
					<text>{{list.gameName}}</text>
					<!-- <text class="iconfont icon-shoucang1-copy"></text> -->
				</view>
				<view class="game-time padding-LR20">
					<text class="game-source">{{list.gameSource}}</text>
					<text>{{list.postTime}}</text>
				</view>
				<view class="game-item-tags animated bounceInRight">
					<view v-for="(tag, i) in list.gameTags" :key="i">
						<uni-tag :text="tag.tagName" size="small" :type="type(i)"></uni-tag>
					</view>
				</view>
			...
			<view>
				...
				<view class="time-box line">
					<text class="cuIcon-time" style="margin-right: 10upx;"></text>
					<text>比赛时间：</text>
					<text>{{isUnknowTime(list.gameStartTime)}} ~ {{isUnknowTime(list.gameEndTime)}}</text>
				</view>
			</view>
			<view class="game-detail">
				<view class="game-detail-topic">
					<text class="iconfont icon-GroupCopy" style="color:#008de1"></text>
					<text class="fontWeight550">竞赛信息</text>
					<text v-if="list.teamSize==1">(个人赛)</text>
					<text v-else>(团体赛)</text>
				</view>
				<view class="game-detail-context">
					<rich-text :nodes="list.gameContent || '还没有发布相关的比赛介绍~'"></rich-text>
				</view>
			</view>
			...
			<view class="game-relative">
				...
				<view style="">
					<view v-if="list.gameUrl!=''" style="padding-left: 50upx;">
						<uni-link :href="list.gameUrl" text="点击复制" copyTips="链接已复制~"></uni-link>
					</view>
					<view v-else style="padding: 20upx 40upx;">
						暂无竞赛链接~
					</view>
			...
			<view class="game-other">
				<view class="game-other-item"  @click="this.$api.msg('暂无公告')">
				...
				<view class="game-other-item line" :class="isPreview==false?'marginBottom100':''" @click="this.$api.msg('暂无附件')">
					<view class="other-link">
						<text class="iconfont icon-GroupCopy" style="color:#008de1"></text>
						<text>赛事附件（0）</text>
					</view>
...
<!-- 			<view class="game-participate">
			...
			<view class="game-footer" v-if="isPreview==false">
				<view class="icons">
					...
					<text v-if="userInfo.user.userType <= 2" @click="editGame" class="cuIcon-edit"></text>
					<text v-if="userInfo.user.userType <= 2" @click="deleteGame" class="cuIcon-delete"></text>
				</view>
