<template>
	<view class="content">
		<hx-navbar title="搜索活动" color="#ffffff" background-color-linear-deg="45" />
		...
		<view class="search-keyword" @touchstart="blur">
			<scroll-view class="keyword-list-box" v-show="isShowKeywordList" scroll-y>
				<view class="keyword-entry" hover-class="keyword-entry-tap" v-for="row in keywordList" :key="row.keyword">
					...
					<view class="keyword-img" @tap="setkeyword(row)" style="border:1upx solid #f00;">
						<image src="http://www.iamk.top/static/icon/cha.png"></image>
					</view>
				</view>
			</scroll-view>
			<scroll-view class="keyword-box" v-show="!isShowKeywordList" scroll-y>
				<view class="keyword-block" v-if="oldKeywordList.length>0">
					<view class="keyword-list-header">
						<view>历史搜索</view>
						<view>
							<image @tap="oldDelete" src="http://www.iamk.top/static/icon/cha.png"></image>
						</view>
					</view>
					<view class="keyword">
	...
	export default {
		...
		methods: {
			...
			doSearch(key) {
				...
				uni.navigateTo({
					url: `/pages/index/game-list/game-list?key=${key}`,
				})
			},
			//保存关键字到历史记录
			saveKeyword(keyword) {
